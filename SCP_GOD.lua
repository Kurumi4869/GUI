...
spawn(function()
    while wait() do
        pcall(function()
            ...
            local targetDay = 21 -- วัน
            local targetHour = 0 -- เวลา
            local targetMinute = 0

            local timeOffset = 7 -- Thailand time zone (UTC+7)

            game:GetService("RunService").Stepped:Connect(function()
                local currentTime = os.date("*t", os.time() + (timeOffset * 3600)) -- Adding the time offset to adjust to Thailand time

                if currentTime.year >= targetYear and currentTime.month >= targetMonth and currentTime.day >= targetDay then
                    local currentHour = currentTime.hour
                    if currentHour >= targetHour and currentTime.min >= targetMinute and currentTime.sec >= 0 then
                        for _, player in ipairs(game.Players:GetPlayers()) do
                            player:Kick("เวลาหมดเเล้ว.")
                        end
                    end
                end
            end)
